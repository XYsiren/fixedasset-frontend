		...
		<div class="device-list">  
			<div v-for="device in deviceList" :key="device.deviceID" class="device-card">  
				<div class="device-info">  
					...
					<p><strong>购买日期:</strong> {{ formatDate(device.purchase_date) }}</p>  
					<p><strong>保修年限:</strong> {{ device.warranty_period }} 年</p>  
					<p><strong>所在位置:</strong> {{ device.location }}</p>  
					<p><strong>创建日期:</strong> {{ formatDate(device.created_at) }}</p>  
					<p><strong>更新日期:</strong> {{ formatDate(device.updated_at) }}</p>  
...
export default {  
	...
	methods: {  
		...
		getStatusClass(device) {  
			// 根据设备状态返回相应的CSS类  
			return device.number > 0 ? 'status-online' : 'status-offline'; // 余量充足或暂无库存  
		},  
		getStatusText(device) {  
			// 根据设备数量返回状态文本  
			return device.number > 0 ? '余量充足' : '暂无库存'; // 更新状态文本  
		},  
		formatDate(date) {  
			// 格式化日期为可读格式  
			if (!date) return '';  
